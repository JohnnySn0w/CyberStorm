...
def solve(HOST, PORT, login, passwd, n):
##################################################################################
	# format: python ftp-cyber.py [HOST] [PORT] [user] [passwd] [directory name]
...
def Main():
	while True:
		if (len(sys.argv) == 4):
			...
			passwd = sys.argv[4]
			n = sys.argv[5]
			solve(HOST, PORT, login, passwd, n)
			exit()
		else:
			print "Error. Try typing your command as follows: python ftp-cyber.py [host] [port] [login name] [password] [directory name]"
			exit()
Main()